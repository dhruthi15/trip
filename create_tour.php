...
$get_field_10_1 = ($data->field_10);
$get_field_10 = substr ($get_field_10_1, 0, 10); 
$get_field_11_1 = ($data->field_11);
$get_field_11 = substr ($get_field_11_1, 0, 10); 
$get_field_12 = "Active"; 
$get_created_date =date('Y-m-d');

if( empty($get_email) ||
	empty($get_field_1) || empty($get_field_2) || empty($get_field_3) ||
	empty($get_field_5) || empty($get_field_6)|| empty($get_field_7)|| empty($get_field_8)  )
{
	$response["success"] = 2;
	echo json_encode($response);
}
else if (strlen($get_field_4) != 10) 
{
	$response["success"] = 3;
	echo json_encode($response);
}
...
{
	$result = mysqli_query($conn,"INSERT INTO tour( email,
